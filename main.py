...
def _publish_entry(category, source_name, entry):
    ...
    # крайнему: og:image со страницы статьи (это та же картинка, что видит
    # человек в источнике, и обычно полный размер) -> картинка из RSS
    # (ленты часто отдают урезанную CDN-копию или мелкий превью) ->
    # релевантное стоковое фото по теме -> и только если вообще ничего
    # не нашли — рисуем сами.
...
def _publish_digest():
    ...
    # Каждая тема — отдельный абзац (двойной перенос строки), а не сплошной
    # список: так сводка читается на телефоне, каждая строка «на своей строке».
