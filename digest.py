...
from config import (WORLD_FEEDS, DIGEST_TOPICS, DIGEST_MIN_TOPICS,
...
DIGEST_PROMPT = (
    ...
    "Начинай строку с области (политика, экономика, технологии, наука, общество, "
    "культура), затем тире и тема. Распределяй темы по областям точно:\n"
    "- природа и погода — землетрясения, наводнения, ураганы, засухи, эпидемии — "
    "это «общество», а НЕ «наука»;\n"
    "- к «науке» относи только исследования, открытия, космос и медицину;\n"
    "- выборы и политика — «политика», бизнес и рынки — «экономика», "
    "программирование и гаджеты — «технологии».\n"
    "Только сами строки: без нумерации, без маркеров списка, без вводных и "
    "заключительных фраз до и после списка.\n"
    "Каждую строку заканчивай точкой.\n"
)
...
# Символы, на которые строка уже «корректно» заканчивается: точку к ним
# дописывать не нужно, чтобы не плодить «..» и не портить «...» от усечения
# длинных строк в _parse_bullets.
_TERMINAL_CHARS = '.!?…'


def _ensure_period(line):
    """Страховка: каждая строка дайджеста должна заканчиваться точкой.
    Инструкция есть и в промпте, но free-модель может её проигнорировать —
    тогда точку дописываем здесь, кодом."""
...
def _parse_bullets(content, limit):
    ...
    for raw in content.splitlines():
        ...
        if not line or not _BULLET_DASH_RE.search(line):
            continue
        # Нормализуем (добавляем точку) до дедупликации: одна и та же тема
        # «с точкой» и «без точки» — это один дубль, а не две разные строки.
        line = _ensure_period(line)
        if len(line) > 400:
            line = line[:397] + '...'
